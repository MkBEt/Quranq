...
window.arabicChapters = Object.keys(chaptersJSON)
window.englishChapters = Object.values(chaptersJSON)


window.capitalize = function capitalize(words) {
  return words.toString().toLowerCase().replace(/(^\w{1})|(\s+\w{1})/g, match => match.toUpperCase()).trim()
}

let htmlparser = new window.DOMParser();
...
window.getQuranCardElem = function (quran, editionName, dirval, lang, authorName, isocodes) {
